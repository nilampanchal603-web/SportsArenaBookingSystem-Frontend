...
import { FiHome, FiMap, FiCalendar, FiClock, FiUsers, FiLogOut, FiBell, FiUser, FiCheckCircle, FiDollarSign } from 'react-icons/fi'
import LogoutModal from '../LogoutModal'

export const ArenaManagerNavbar = () => {
  ...
  const navItems = [
    { name: 'Dashboard', path: '/arenamanager/dashboard', icon: <FiHome className="text-xl" /> },
    { name: 'Manage Arenas', path: '/arenamanager/arenas', icon: <FiMap className="text-xl" /> },
    { name: 'Booking Requests', path: '/arenamanager/bookingrequests', icon: <FiCheckCircle className="text-xl" /> },
    { name: 'Schedule', path: '/arenamanager/schedule', icon: <FiCalendar className="text-xl" /> },
    { name: 'Coach Availability', path: '/arenamanager/availability', icon: <FiClock className="text-xl" /> },
    { name: 'Manage Coaches', path: '/arenamanager/coaches', icon: <FiUsers className="text-xl" /> },
    { name: 'Coach Earnings', path: '/arenamanager/coach-earning', icon: <FiDollarSign className="text-xl" /> },
  ];
      ...
      <div className="w-64 bg-[#0F172A] text-white flex flex-col fixed h-full shadow-xl z-20 transition-all duration-300">
        <div className="p-6 border-b border-[#1F2937]">
          <h1 className="text-2xl font-bold text-white tracking-wide">
            ...
            <span className="block text-xs font-normal text-[#6B7280] mt-1 uppercase tracking-widest">Arena Manager</span>
          </h1>
        </div>
      ...
      <div className="ml-64 flex flex-col w-full min-h-screen">
        ...
        <header className="bg-white border-b border-[#E5E7EB] h-16 flex items-center justify-between px-8 sticky top-0 z-10 shadow-sm">
          <h2 className="text-xl font-semibold text-[#0F172A]">
            {navItems.find(item => location.pathname.includes(item.path))?.name || 'Arena Manager Dashboard'}
          </h2>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm font-medium text-[#1F2937] hidden md:block">Manager</span>
            </div>
          </div>
        </header>
